...
public class Polygon {
    ...
    public Optional<Polygon> clip(Line clipLine) {
        ...
        for (int i = 0; i < numPoints; i++) {
            ...
            Line.IntersectionResult intersection = Line.findIntersection(line, clipLine);

            if ((intersection.type == Line.IntersectionType.SKEW && (intersection.t < 0 || intersection.t > 1)) || intersection.type == Line.IntersectionType.PARALLEL || intersection.type == Line.IntersectionType.SAME) {
                if (clipLine.classifyPoint(x2, y2) != Line.PointType.LEFT) {
                ...
                if (clipLine.classifyPoint(x2, y2) == Line.PointType.RIGHT) {
                    xPointsNewList.add(x2);
                    yPointsNewList.add(y2);
                }
            }
        ...
        if (!xPointsNewList.isEmpty()) {
            ...
            return Optional.of(new Polygon(xPointsNewArray, yPointsNewArray));
        }

        return Optional.empty();
    }

    public Optional<Polygon> clip(Polygon clipPolygon) {
        ...
        Optional<Polygon> polygonNew = Optional.of(new Polygon(xPoints, yPoints));

        int numPoints = clipPolygon.getNumPoints();
        for (int i = 0; i < numPoints; i++) {
            if (polygonNew.isPresent()) {
                Line clipLine = new Line(clipPolygon.xPoints[i], clipPolygon.yPoints[i], clipPolygon.xPoints[(i + 1) % numPoints], clipPolygon.yPoints[(i + 1) % numPoints]);
