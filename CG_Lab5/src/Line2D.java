...
public class Line2D {
    ...
    public static IntersectionResult findIntersection(Line2D line1, Line2D line2) {
    ...
    public static IntersectionType classifyCross(Line2D line1, Line2D line2) {
    ...
    public Optional<Line2D> clip(Polygon2D clipPolygon) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (den != 0) {
                ...
                Line2D line = new Line2D(px[i], py[i], px[(i + 1) % n], py[(i + 1) % n]);
                Line2D.PointType pointType = line.classifyPoint(x1, y1);
        ...
        if (t1 <= t2) {
            ...
            return Optional.of(new Line2D(x1_new, y1_new, x2_new, y2_new));
        }
